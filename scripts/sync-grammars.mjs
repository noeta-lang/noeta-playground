 ...
 * language repo. Runs as the predev/prebuild hook so `pnpm dev` and
 * `pnpm build` work without manual setup; `syntaxes/` is generated and
 * gitignored.
 *
 ...
 *     shiki registration in src/lib/noeta-highlight.mjs.
 *
 * Two sources, in order of preference (same shape as the docs repo's
 * sync-grammars.mjs):
...
const snapshot = () => GRAMMARS.map((g) => read(join(outDir, g)));
...
try {
  ...
  // Astro caches build artifacts in node_modules/.astro and only invalidates
  // on source changes — a grammar-only change could otherwise ship pages with
  // stale highlighting. Bust the cache when the synced grammars changed.
  const changed = snapshot().some((g, i) => g !== before[i]);
  if (changed && existsSync(resolve(repoRoot, "node_modules", ".astro"))) {
    console.log("[sync-grammars] Grammars changed; clearing the Astro cache");
    await rm(resolve(repoRoot, "node_modules", ".astro"), { recursive: true, force: true });
  }
  console.log(
